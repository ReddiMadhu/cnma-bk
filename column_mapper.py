...
RMS_CANONICAL_FIELDS: Dict[str, List[str]] = {
    ...
    "WALLTYPE":     ["wall type", "wall"],
    # Peril values — passed through directly from source
    "EQCV1VAL":          [
        "bldg",
        "bldg value",
        "building limit",
...
class ColumnMappingState(TypedDict):
    ...
    fuzzy_results: Dict[str, List[Dict]]    # col → [{canonical, score, method}]
    llm_candidates: List[str]              # cols needing LLM
    llm_results: Dict[str, Dict]           # col → {match, confidence, reason}
