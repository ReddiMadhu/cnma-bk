"""
code_mapper.py — 6-stage construction/occupancy code mapping via LangGraph.

Stage 0  : ISO Fire Class fast-path (scheme="ISO" or exact ISO label detected)
Stage 0.5: RMS→AIR cross-translation fast-path (target=AIR, scheme=RMS, exact RMS code)
...
LangGraph graph: iso_direct → rms_direct → conflict → deterministic → llm → tfidf → default → END
...
LLM Reliability Notes:
  ...
  - Retry-with-fallback (max 2 retries → TF-IDF on failure)
...
# ── Gemini rate limiter (15 calls / 60 s sliding window) ───────────────────────
...
_REF_DIR = _BASE / "reference"
_TFIDF_DIR = _BASE / "tfidf_cache"
...
_iso_map: Dict[str, Dict] = {}              # iso_class_key → {air_class, aliases, ...}
...
# i.e. scheme="ATC", value="10" → Entertainment (AIR 317)
...
# occupancy field — see _is_atc_scheme() for field-aware logic.
...
class _LLMCodeResult(BaseModel):
    ...
    code: str = Field(
        description=(
            ...
            "Must be an exact code string from the list — never invent codes."
        )
    )
...
class _LLMBatchResult(BaseModel):
    """Batch result wrapper — one entry per input item."""
    items: List[_LLMCodeResult]


# ── Gemini model (configured at startup after codes are loaded) ────────────────
...
MANDATORY CONFLICT RESOLUTION RULES (apply in this exact priority order):
  RULE 1 — URM GOVERNS (WEAKEST): Unreinforced masonry load-bearing walls ALWAYS govern
           regardless of secondary steel roof or other elements. → Joisted Masonry
  RULE 2 — MIXED CONSTRUCTION: Two distinct load-bearing structural systems in one building
           OR combustible upper floors over concrete/masonry base → Mixed Construction
  RULE 3 — COMBUSTIBLE DOWNGRADE: Wood roof/joists/trusses on masonry walls
           (not just insulation or partition) → Joisted Masonry
...
def build_tfidf_indexes() -> None:
    ...
    logger.info(
        ...
        f"{len(_iso_map)} ISO classes, {len(_rms_to_air_map)} RMS→AIR mappings."
    )
...
def _lookup_atc(value: str) -> Optional[Dict]:
    """Look up ATC class value in the ATC→AIR map."""
...
# ── RMS→AIR cross-translation lookup ─────────────────────────────────────────
...
# ── Priority-weighted deterministic scoring ────────────────────────────────────
...
def _score_code_candidate(text: str, code: str, meta: Dict) -> float:
    ...
    Scoring tiers:
      1.0 — exact full text match to keyword
      0.7 — keyword phrase found in text (phrase-in-text)
      0.5 — text found in keyword phrase (text-in-phrase)
      0.3 — description phrase in text
      0.2 — single-word overlap in keyword list
...
# ── LLM helper: retry-with-fallback ───────────────────────────────────────────
...
def _call_gemini_with_retry(
    ...
    for attempt in range(max_retries + 1):
        try:
            _rate_limit_gemini()   # enforce ≤15 calls/min before each attempt
            ...
            # Secondary code validity check — reject any hallucinated codes
    ...
    for item in state["unique_items"]:
        ...
        if atc_data:
            ...
            results[str(idx)] = {
                ...
                "original": raw_value,
                "alternatives": [],
                "reasoning": f"ATC class '{raw_value}' ({atc_data.get('description', '')}) → AIR {air_code}",
                "abbreviation_expanded": False,
                "atc_class": raw_value,
            }
            logger.debug(f"ATC direct: item {idx} ATC '{raw_value}' → AIR {air_code}")
        else:
            pending_iso.append(idx)
...
# ── Stage 0.5: RMS→AIR cross-translation fast-path ────────────────────────────

def _node_rms_direct(state: CodeMappingState) -> CodeMappingState:
    """
    Fast-path for RMS→AIR construction cross-translation (construction field only).
    ...
    to the conflict stage — the construction RMS cross-translation table
    ...
    for item in state["unique_items"]:
        ...
        # Occupancy: RMS = ATC (same system) — already handled by _node_atc_direct.
        ...
        # Construction only: apply RMS→AIR cross-translation when target=AIR
...
# ── Stage 1: Structural conflict resolver ─────────────────────────────────────
...
def _node_conflict(state: CodeMappingState) -> CodeMappingState:
    ...
    for idx in pending:
        ...
        if conflict_result and conflict_result.confidence >= threshold:
            ...
            # No conflict detected → pass to deterministic
            pending_llm.append(idx)

    return {**state, "results": results, "pending_llm": pending_llm, "conflict_hints": conflict_hints}


# ── Stage 2: Priority-weighted deterministic keyword matching ─────────────────
...
def _node_deterministic(state: CodeMappingState) -> CodeMappingState:
    ...
    for idx in pending_llm:
        ...
        # ── Occupancy: raw-string cache + context rules (pre-scoring) ────────
        if state["field"] == "occupancy":
        ...
        if best_code and best_meta:
            results[str(idx)] = {
                ...
                "original": raw_value,
                "alternatives": alts,
                "reasoning": f"Priority-weighted keyword match (score→confidence {confidence:.2f})",
                "abbreviation_expanded": was_expanded,
            }
...
# ── Stage 3: Gemini LLM with structured output ────────────────────────────────
...
def _node_llm(state: CodeMappingState) -> CodeMappingState:
    ...
    - Retry-with-fallback (2 retries → TF-IDF)
...
OCCUPANCY-SPECIFIC RULES (additional, apply after structural rules):
  OCC-1: "Shop" in county/municipal/government context → General Services (343)
         "Shop" standalone → Retail (312); "Mechanic/Auto Shop" → Personal Services (314)
  OCC-2: "Pharmacy/Drug Store" standalone → Retail (312);
         "Pharmacy" inside hospital → Health Care (316)
  OCC-3: Multi-use: primary function governs; residential over commercial if policy is residential
  OCC-4: Strip tenant names (Amazon, Walmart, McDonald's) — classify by building function
  OCC-5: School level: Elementary/Primary/K-12/Daycare → Primary (346); University/College → Higher Ed (345)
  OCC-6: BBQ structures, outdoor kitchens, grills → Restaurants (331)
  OCC-7: Utility subtypes: pumping/water treatment → Water (362); sewer/wastewater → Sewer (363);
         gas distribution → Natural Gas (364); power/electric → Electrical (361)
  OCC-8: All farm subtypes (dairy, poultry, grain, ranch, livestock) → Agriculture (373)
  OCC-9: "Activity Center" in government context → General Services (343); recreational → Entertainment (317)
  OCC-10: Casinos → Entertainment (317) using ATC 48; Parking garage → Parking (318)
"""
    ...
    if parsed and len(parsed.items) == len(pending):
        for i, idx in enumerate(pending):
            ...
            if code in registry:
                ...
                logger.warning(f"LLM code '{code}' not in registry for item {idx}. → TF-IDF.")
                pending_tfidf.append(idx)
    else:
        if not parsed:
            ...
            logger.warning(f"LLM returned {len(parsed.items)} items but expected {len(pending)}. → TF-IDF all.")
...
# ── Build and compile the 6-stage graph ───────────────────────────────────────
...
def map_codes(
    ...
    Stages:
      0   iso_direct    — ISO Fire Class 0-9 fast-path (confidence 1.0)
      0.5 rms_direct    — RMS→AIR cross-translation (confidence 0.98)
      1   conflict      — Structural conflict resolution (compound descriptions)
      2   deterministic — Priority-weighted keyword scoring
      3   llm           — Gemini with response_schema + temperature=0
      4   tfidf         — Cosine similarity fallback
      5   default       — Business rules default

    unique_items: [{index: int, scheme: str, value: str, context: {year_built, stories}}]
    Returns: {index_str → result_dict}
    """
